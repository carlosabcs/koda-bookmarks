...
chrome.runtime.onMessage.addListener(
  (message: ExtensionMessage, sender, sendResponse) => {
    ...
    if (message.action === ACTIONS.SAVE_BOOKMARK) {
      ...
      if (existingId) {
        chrome.bookmarks.update(existingId, { title, url }).then(() => {
          chrome.bookmarks.move(existingId, { parentId }).then(() => sendResponse({ success: true }));
        });
        return true;
      } 
      
      chrome.bookmarks.create({ parentId, title, url }).then(() => sendResponse({ success: true }));
      return true;
    }
    ...
    if (message.action === ACTIONS.CREATE_FOLDER_AND_SAVE) {
      ...
      chrome.bookmarks.create({ parentId, title: folderName }).then((newFolder) => {
        if (existingId) {
          chrome.bookmarks.update(existingId, { title, url }).then(() => {
            chrome.bookmarks.move(existingId, { parentId: newFolder.id }).then(() => sendResponse({ success: true }));
          });
          return;
        }
        
        chrome.bookmarks.create({ parentId: newFolder.id, title, url }).then(() => sendResponse({ success: true }));
      });
      return true;
    }
  }
